...
- kgraph 连通图：**整体楼层空间已联通**（跨层/穿门/楼梯可达；以线上场景返回结果为准）
...
**连通图枚举接口不开放**：get-reachable-graph 仍不作为图规模盘点接口；导航统一走 shortest-path-with-waypoints，当前整体楼层空间已联通。

## 「内容显隐 → 场景包内容」面板改造设计输入

数据基座已就绪（analyzeScenePack）。建议面板双页签：
1. **内容显隐**（现有开关，保持）
2. **场景包内容**（新）：总览卡（总节点/类型数/楼层数）+ 类型表（数量+Site 级占比，点击行=跳显隐开关）+ 楼层×类型矩阵（热力，异常层高亮）+ Space 语义分类条 + Site 级对象清单（出入口/车辆/屋顶设备）+ 整体楼层空间连通注记
