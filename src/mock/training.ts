...
const CENTER = { lng: 118.7968, lat: 32.0603 };
...
const BY_FLOOR: FamiliarNode[] = [
  node('byFloor', ['bf-b2-garage', 'B2 车库', '地下区域', 'B2', '地下二层', undefined, 68,
    ['B2 车库建筑面积约 8600㎡，划分 4 个防火分区，车位 212 个',
     '车库出入口位于建筑北侧，坡道宽 7m，可供消防车通行至 B1',
     '设有柴油发电机房与储油间，为重点防火部位'],
    ['室内消火栓', '喷淋系统', '排烟口', '防火卷帘'],
    ['车库防火分区最大允许建筑面积 2000㎡（设自喷可加倍）', '储油间储油量不应大于 1m³']]),
  node('byFloor', ['bf-b1-equip', 'B1 设备层', '地下区域', 'B1', '地下一层', undefined, 55,
    ['B1 集中布置变配电室、消防水泵房、生活水泵房',
     '消防水泵房设 2 台消火栓泵（一用一备）、2 台喷淋泵',
     '疏散通道直通防烟楼梯间，疏散距离不大于 40m'],
    ['消防水泵', '变配电室', '挡水门槛'],
    ['消防水泵房疏散门应直通安全出口', '变配电室应采用甲级防火门']]),
  node('byFloor', ['bf-1f-lobby', '1F 大堂', '低区', '1F', '首层', undefined, 82,
    ['1F 大堂净高 9m，主入口朝南，设 3 个安全出口直通室外',
     '消防控制室位于 1F 东侧，有直接通往室外的出口',
     '大堂两侧为防烟楼梯间 A/B 首层入口'],
    ['消防控制室', '安全出口', '手动报警按钮'],
    ['消防控制室应设在首层或地下一层', '常考：消控室疏散门应直通室外']]),
  ...
  node('byFloor', ['bf-3-5f-dining', '3-5F 餐饮', '低区', '3-5F', '裙楼', undefined, 39,
    ['3-5F 为餐饮层，共 14 家商户，后厨集中布置于东侧',
     '3F 后厨区域设燃气表间，为重点部位',
     '厨房排油烟管道每季度清洗，管道井防火封堵完好'],
    ['燃气表间', '厨房灭火装置', '烟感'],
    ['厨房应设厨房设备灭火装置', '燃气表间应设可燃气体探测器']]),
  node('byFloor', ['bf-6-15f-office', '6-15F 办公', '中区', '6-15F', '塔楼', undefined, 61,
    ['6-15F 标准办公层，每层面积 1680㎡，2 个防火分区',
     '每层设室内消火栓 4 个、灭火器箱 4 具，位于电梯厅两侧',
     '疏散走道净宽 1.4m，双向疏散'],
    ['室内消火栓', '灭火器箱', '疏散指示'],
    ['办公层疏散走道净宽不应小于 1.3m', '常考：消火栓间距不大于 30m']]),
  node('byFloor', ['bf-16f-refuge', '16F 避难层', '中区', '16F', '塔楼', undefined, 72,
    ['16F 避难层净面积 620㎡，可容纳约 1200 人，设独立机械加压送风',
     '避难层与设备层合用，设备管道区采用耐火极限 3.00h 隔墙分隔',
     '设消火栓、消防专线电话、应急广播与疏散指示标志'],
    ['机械加压送风口', '消防电话', '消火栓'],
    ['常考：避难层间距不应大于 50m', '避难层净面积按 5 人/㎡ 计算']]),
  node('byFloor', ['bf-17-30f-office', '17-30F 办公', '高区', '17-30F', '塔楼', undefined, 34,
    ['17-30F 为高区办公层，由高速电梯组服务，消防电梯可达',
     '高区由屋顶水箱间与中间转输水箱联合供水',
     '30F 以上疏散可向上至屋顶停机坪'],
    ['消防电梯', '转输水箱', '正压送风口'],
    ['高区消火栓栓口动压不应大于 0.50MPa', '超过 100m 建筑应设屋顶直升机停机坪']]),
  node('byFloor', ['bf-31-32f-obs', '31-32F 观光层', '高区', '31-32F', '塔楼顶部', undefined, 58,
    ['31-32F 为观光层，人员密集，营业时间至 22:00',
     '设 2 部防烟楼梯间与 1 部专用疏散电梯',
     '全层设智能疏散指示系统，与火灾报警联动'],
    ['疏散电梯', '智能疏散指示', '排烟窗'],
    ['人员密集场所疏散门应向疏散方向开启', '观光层应核定最大容纳人数']]),
  node('byFloor', ['bf-roof-helipad', '屋顶直升机停机坪', '高区', '屋顶', '屋面', undefined, 76,
    ['屋顶停机坪直径 21m，承重满足直-9 级别直升机起降',
     '四周设航空障碍灯、边界灯与风向标，设消火栓与泡沫灭火设施',
     '停机坪疏散通道直通 32F，宽度不小于 0.9m'],
    ['停机坪边界灯', '消火栓', '泡沫栓'],
...
const BY_FACILITY: FamiliarNode[] = [
  node('byFacility', ['fc-pool', '消防水池', '供水类', 'B2', '水泵房旁', 2, 80,
  ...
  node('byFacility', ['fc-control-room', '消防控制室', '报警类', '1F', '东侧', 1, 88,
    ['消防控制室位于 1F 东侧，24h 双人持证值班',
...
const BY_KEY_PART: FamiliarNode[] = [
  node('byKeyPart', ['kp-control-room', '消防控制室', '重点部位', '1F', '东侧', 1, 88,
    ['24h 双人持证值班，值班记录完整',
     '报警控制器处于自动联动状态，屏蔽点位每月复核',
     '备用电源可维持 3h 以上供电'],
    ['报警控制器', '联动控制盘', '应急广播'],
    ['常考：消控室值班制度', '故障屏蔽点应登记并限期恢复']]),
  node('byKeyPart', ['kp-power', '变配电室', '重点部位', 'B1', '北侧', 1, 49,
    ['10kV 变配电室，设 2 台 1600kVA 干式变压器',
     '配置全淹没气体灭火系统与感温电缆',
  ...
  node('byKeyPart', ['kp-diesel', '柴油发电机房 + 储油间', '重点部位', 'B2', '西南角', 1, 53,
    ['柴油发电机组 1200kW，市电中断 15s 内自动启动',
     '储油间储油量 0.8m³，采用防火隔墙与甲级防火门分隔',
     '机房设水喷雾灭火系统与事故通风'],
    ['水喷雾系统', '储油间', '甲级防火门'],
    ['储油间储油量不应大于 1m³', '发电机应 15s 内自启动']]),
  node('byKeyPart', ['kp-refuge', '避难层（16F / 30F）', '重点部位', '16F/30F', '塔楼', 2, 72,
    ['16F 净面积 620㎡ 可容纳约 1200 人；30F 净面积 580㎡ 可容纳约 1100 人',
...
export function findNode(id: string): FamiliarNode | undefined {
  return FAMILIAR_NODES.find((n) => n.id === id);
}
...
const BANK_COMMANDER: ExamQuestion[] = [
  q('commander', 1, 'multiple', '建筑层数 · 避难层设置', '金茂大厦避难层设置在下列哪些楼层？（多选）',
    ['8F', '16F', '24F', '30F'], [1, 3],
    '避难层间距不应大于 50m，32 层建筑设 16F、30F 两层避难层。', 'bf-16f-refuge'),
  q('commander', 2, 'single', '力量部署 · 首战力量', '金茂大厦 5F 餐饮厨房发生火灾，首战力量应优先调派哪些车辆？',
  ...
  q('commander', 5, 'multiple', '疏散组织 · 避难层', '组织人员向 16F 避难层疏散时，指挥员应掌握的信息包括（多选）：',
    ['避难层净面积与可容纳人数', '加压送风是否启动', '避难层内商户营业额', '通往避难层的疏散通道数量'], [0, 1, 3],
    '避难层容量、防烟状态、疏散通道是指挥决策关键要素。', 'bf-16f-refuge'),
  ...
  q('commander', 10, 'single', '重点部位 · 柴油发电机房', 'B2 柴油发电机房火灾，应优先选用的灭火系统是？',
  ...
  q('commander', 12, 'single', '力量部署 · 增援调度', '火势蔓延至 16F 避难层邻近楼层时，增援力量应重点加强哪个方向？',
...
const BANK_FIGHTER: ExamQuestion[] = [
  ...
  q('fighter', 3, 'multiple', '设施操作 · 避难层', '16F 避难层内可供灭火救援使用的设施包括（多选）：',
    ['室内消火栓', '消防专线电话', '应急广播', '自动售货机'], [0, 1, 2],
    '避难层设消火栓、消防电话、应急广播与疏散指示。', 'bf-16f-refuge'),
  ...
  q('fighter', 10, 'single', '灭火进攻 · 车库火灾', 'B2 车库汽车火灾，进攻通道应选择？',
    ['北侧坡道或防烟楼梯间', '客运电梯', '自动扶梯', '采光井垂直进入'], [0],
    '车库出入口坡道可供消防车至 B1，内攻走防烟楼梯间。', 'bf-b2-garage'),
  q('fighter', 11, 'multiple', '设施操作 · 消火栓', '室内消火栓箱内标准配置包括（多选）：',
    ['25m 水带', '水枪', '消防软管卷盘', '防毒面具'], [0, 1, 2],
    '栓箱标配水带、水枪与软管卷盘，不含防毒面具。', 'fc-indoor-hydrant'),
  q('fighter', 12, 'single', '灭火进攻 · 避难层搜救', '对 31-32F 观光层进行人员搜救时，可利用的疏散途径不包括？',
    ['防烟楼梯间', '专用疏散电梯（消防员控制）', '普通客运电梯', '屋顶停机坪'], [2],
    '火灾时普通客梯停止使用，疏散靠楼梯间、消防电梯与停机坪。', 'bf-31-32f-obs'),
];

const BANK_DRIVER: ExamQuestion[] = [
  q('driver', 1, 'single', '车辆停靠 · 高层建筑', '举高喷射车在金茂大厦周边作业时，停靠位置应选择？',
  ...
  q('driver', 5, 'judge', '车辆停靠 · 车库', '水罐车可经北侧坡道直接驶入 B2 车库出水灭火。',
    JUDGE_OPTS, [1],
    '坡道仅通至 B1，B2 层高与转弯条件不满足消防车进入。', 'bf-b2-garage'),
