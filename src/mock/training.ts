...
// 按建筑层数
// 分区口径（2026-08-24 统一）：低区 1-13F / 中区 14-25F（25F 避难层属中区）/ 高区 26-40F。
// 业态口径：预案部位优先（1F 大堂/2F 商业/3-4F 餐饮/5-6F 影院/B1 车库+设备）+ 三业态共存
// （2-12F 办公室 / 14-24F 房间+机房+厨房+配电间 / 26-40F 客房+布草间+机房）。
const BY_FLOOR: FamiliarNode[] = [
  ...
  node('byFloor', ['bf-6-15f-office', '低区办公（2-12F）', '低区', '2-12F', '塔楼', undefined, 61,
    ['2-12F 为低区办公层，以各类办公室为主',
     '每层面积 1680㎡，划分 2 个防火分区',
     '每层设室内消火栓 4 个、灭火器箱 4 具，位于电梯厅两侧',
     '疏散走道净宽 1.4m，双向疏散'],
    ['室内消火栓', '灭火器箱', '疏散指示'],
    ['办公层疏散走道净宽不应小于 1.3m', '常考：消火栓间距不大于 30m']]),
  node('byFloor', ['bf-13f-refuge', '13F 避难层', '低区', '13F', '塔楼', undefined, 72,
  ...
  node('byFloor', ['bf-17-30f-office', '中区配套（14-24F）', '中区', '14-24F', '塔楼', undefined, 34,
    ['14-24F 为中区配套层，含房间、机房、厨房、配电间',
     '本区疏散中转依托 13F 避难层，向上可达 25F 避难层',
     '中区由中间转输水箱接力供水，消火栓系统分区运行',
     '由高速电梯组服务，消防电梯可达'],
    ['消防电梯', '转输水箱', '正压送风口'],
    ['高区消火栓栓口动压不应大于 0.50MPa', '避难层上下层为重点设防部位']]),
  node('byFloor', ['bf-25f-refuge', '25F 避难层', '中区', '25F', '塔楼', undefined, 70,
    ['25F 避难层净面积 580㎡，可容纳约 1100 人，设独立机械加压送风',
     '为高区疏散中转，与 13F 避难层间距小于 50m',
     '设消火栓、消防专线电话、应急广播与疏散指示标志'],
    ['机械加压送风口', '消防电话', '消火栓'],
    ['避难层不得布置可燃物库房', '常考：避难层间距不应大于 50m']]),
  node('byFloor', ['bf-31-32f-obs', '高区客房（26-40F）', '高区', '26-40F', '塔楼高区', undefined, 58,
    ['26-40F 为高区客房层，含客房、布草间、机房',
     '25F 避难层为本区疏散中转',
     '高区供水由转输水箱接力，消火栓栓口动压超限楼层设减压稳压型',
     '40F 为顶层，疏散以向下经防烟楼梯间为主'],
    ['消防电梯', '转输水箱', '正压送风口'],
    ['高区消火栓栓口动压不应大于 0.50MPa', '超高层应设避难层（间距不大于 50m）']]),
];
...
const BY_FACILITY: FamiliarNode[] = [
  ...
  node('byFacility', ['fc-fire-elevator', '消防电梯', '疏散类', '全楼', '核心筒', 4, 66,
    ['消防电梯 4 部，停靠 B1-40F 全部楼层，载重 1000kg',
...
const KNOWLEDGE_NODES: FamiliarNode[] = [
  ...
  node('byKeyPart', ['area-building-role', '本建筑属性定位', '辖区与处置', undefined, '乐盈广场', undefined, 66,
    ['乐盈广场 21 号楼:40F+B1 钢混结构,高 150m,为责任区标志性高层建筑',
     '13F/25F 为避难层,B1 设消控室/消防泵房/变配电间,1-6F 为商业/餐饮/影院,2-12F 办公,14-24F 配套用房,26-40F 客房',
...
const BANK_COMMANDER: ExamQuestion[] = [
  ...
  q('commander', 2, 'single', '力量部署 · 首战力量', '乐盈广场21号楼 4F 餐饮厨房发生火灾，首战力量应优先调派哪些车辆？',
  ...
  q('commander', 8, 'single', '战术决策 · 避难层', '组织 26-40F 高区客房层人员疏散时，首选的疏散中转是？',
    ['25F 避难层', '屋顶', '13F 避难层', '首层大堂'], [0],
    '高区疏散中转依托 25F 避难层，与 13F 避难层间距小于 50m。', 'bf-25f-refuge'),
...
const BANK_FIGHTER: ExamQuestion[] = [
  ...
  q('fighter', 12, 'single', '灭火进攻 · 避难层搜救', '对 26-40F 高区客房层进行人员搜救时，可利用的疏散途径不包括？',
    ['防烟楼梯间', '消防电梯（消防员控制）', '普通客运电梯', '避难层中转'], [2],
    '火灾时普通客梯停止使用，疏散靠防烟楼梯间、消防电梯与避难层中转。', 'bf-31-32f-obs'),
];
...
const BANK_DRIVER: ExamQuestion[] = [
  ...
  q('driver', 10, 'single', '供水保障 · 避难层', '向 13F 避难层消火栓管网补水时，车辆供水应依托的固定设施是？',
    ['水泵接合器（按分区标识对接）', '屋顶水箱直接放水', '地下车库集水坑', '雨水管网'], [0],
    '避难层消火栓由管网供水，车泵经水泵接合器按低区/高区标识补压。', 'fc-adapter'),
...
const BANK_SIGNALER: ExamQuestion[] = [
  ...
  q('signaler', 12, 'single', '通信联络 · 避难层', '组织人员进入 25F 避难层后，通信员应确认建立的通信链路是？',
    ['避难层与消控室的消防专线电话', '避难层人员个人手机', '物业对讲民用频道', '楼顶广播喊话'], [0],
    '避难层设消防专线电话直通消控室，须第一时间确认链路畅通。', 'fc-refuge-2'),
];
